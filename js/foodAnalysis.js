...
async function saveAnalysisResults(analysisData) {
    try {
        ...
        // Save to Firestore
        await firebase.firestore().collection('analysis_history').add({
            userId: userId,
            timestamp: timestamp,
            data: analysisData
        });
        ...
        if (analysisData.imageUrl) {
            const imageRef = firebase.storage().ref().child(`analysis_images/${userId}/${timestamp}`);
...
// Modify the handleImageAnalysis function to save results
export async function handleImageAnalysis(file, language) {
    try {
        // Get the analysis results from Gemini API
        const analysisResults = await handleImageAnalysis(file, language);
        ...
        const analysisData = {
            nutrition: {
                ...
                fat: analysisResults.fat
            },
            foodItems: analysisResults.foodItems.map(item => ({
                name: item.name,
                confidence: item.confidence
            })),
            imageUrl: file ? await convertFileToBase64(file) : null
        };
...
function displayResults(results) {
    ...
    if (calories) {
        calories.innerHTML = `
            ...
            <ul class="list-group">
                <li class="list-group-item">Calories: ${results.calories || 'N/A'}</li>
                <li class="list-group-item">Protein: ${results.protein || 'N/A'}g</li>
                <li class="list-group-item">Carbs: ${results.carbs || 'N/A'}g</li>
                <li class="list-group-item">Fat: ${results.fat || 'N/A'}g</li>
            </ul>
        `;
    }
}